...
- **Темы:** **dark-only** (brand.md — locked решение); `useColorScheme` игнорируется, принудительный тёмный режим через `colorScheme="dark"` в корневом провайдере.
...
- **Battery empty** при тесте → paywall-оверлеем; закрытие без покупки → возврат в курс, прогресс урока сохранён (§12.9, Gap #3/#4), не краш.
- **Недостаточно алмазов на автозаморозку стрика** → стрик сбрасывается (магазина/ручной покупки нет; §A5); баланс не уходит в минус.
...
Офлайн-режим, FCM/APNs пуши, RTDN/расширенный server billing, Cloudflare R2 — см. concept 1.24. Заводятся после релиза MVP. **iOS — в MVP** (целевая платформа наравне с Android, на едином RN-коде). **Квесты (блок F) и магазин убраны из проекта целиком** (квесты — глобальное решение; магазин — КРИТ-8). CI/CD реализуется через EAS уже на этапе разработки (не откладывается).
