...
export interface Streak {
  current: number;
  longest: number;
  // freezesAvailable убран (A-1): магазина/ручной заморозки нет (КРИТ-8) —
  // заморозка автоматическая за алмазы (§A5), «валюта заморозки» = diamonds.
}
...
export interface Course {
  ...
  coverImageUrl: string | null;       // nullable в БД (A-4); маппер коалесит null → placeholder (A13)
  illustrationUrl: string | null;     // главный логотип карточки Lores
  // cardDecorationUrl убран (ревью #10/#11): карточка Lore разрежена — декор не рендерится (§12.3а).
...
export interface EconomyConfig {
  ...
  readingTtsEnabled: boolean;                    // спящий тумблер TTS-озвучки чтения (§6.4/§12.5а, D-1)
...
**Кнопка «назад» (стрелка в хедере).** На **запушенных** экранах (не корни вкладок) сверху слева — иконка-стрелка **`CaretLeft`** (Phosphor `Bold`, цвет — токен `foreground`), тап → `navigation.goBack()`. Применяется на:
...
```json
{
  ...
  "tabs": [
    ...
    { "id": "grid", "icon": "grid", "label_key": "nav_discover", "enabled": true, "order": 3,
...
- **Иконки вкладок** — мапинг `icon`-строки → компонент иконки (из `phosphor-react-native`, вес `Bold`); неизвестная иконка → дефолтная.
...
- **Состав (слева направо, фикс-порядок): `level` · `streak` · `diamonds` · `battery/ассет`.** Уровень в HUD **есть** (подтверждено, ВАЖН-3); §12.11 (Profile) HUD **не дублирует**.
  - **level** — кастомная **статичная цветная SVG** (filled badge/star, oxblood + намёк золота) + число `knowledge_level`.
  - **streak** — кастомная **статичная цветная SVG** (огонь, токен `streak` `#F5A524`) + число `current_streak`.
  - **diamonds** — кастомная **статичная цветная SVG** (гранёный алмаз, токен `diamond` `#2DD4BF`) + баланс.
  - **battery/ассет** — **не-подписчик:** **анимированная SVG-батарея** (единственная иконка с анимацией в MVP, см. ниже); **подписчик:** ассет из `app_config.subscription_badge` (Rive/Lottie/SVG/PNG — уже кастомный, §10).
- **Гемификационные иконки (MVP — без Rive, §brand.md §6).** 4 ключевые иконки — кастомные **filled-цветные SVG** (`react-native-svg` + `react-native-svg-transformer`; в самих SVG `fill="currentColor"` → цвет из токенов кита). Тонкий outline-Lucide для алмаза/огня/батареи **не используется** (выглядит дёшево рядом с Cinzel — ревью #8). Служебный UI (навигация, настройки, замки уроков, галочки, шевроны, стрелка-назад) — **Phosphor Bold** (плотный вес под бренд; точечно `Duotone`), не тонкий Lucide. **Rive отложен пост-MVP** — слоты HUD и привязка данных Rive-ready: позже меняется только реализация компонента `<Svg>` → `<Rive>`, без правок верстки и логики.
- **Анимированная SVG-батарея (без Rive).** Заливка тела батареи — `<Rect>` внутри SVG, высота анимируется через **Reanimated** (`withTiming`, `dur-slow` 350 ms, §brand.md §7):
  - **Разрядка/зарядка** — высота заливки плавно едет вниз/вверх при смене `battery` (доля `battery / batteryMax`, нормализованная — не абсолют, чтобы переживать смену `batteryMax` из админки).
  - **Цвет по тиру (§brand.md §4.2, 3 ступени)** — переключается **по абсолютному значению** (точный порог `≤3`): `battery ≤ 3` → `battery-low` `#E5383B` (кровь); `≤ batteryMax*0.4` → `battery-mid` `#F5A524` (янтарь); иначе → `battery-normal` `#3FB950`. Заливка-высота едет плавно, цвет переключается ступенчато.
  - Остальные 3 иконки (`level`/`streak`/`diamonds`) в MVP **статичны** — анимируется только батарея (единственная часто меняющаяся).
- **Источник данных:** доменный `UserProfile` (§9) собирается из **`profiles` + `streaks` + `subscriptions` + `app_config.subscription_badge`** — одним вызовом `get_me` (§8.1, A-3/C-6) на старте, далее точечные чтения `profiles`/`streaks` через TanStack Query. Поля `isSubscriber` (= `subscriptions.status ∈ trial/active`) и `subscriptionBadge` берутся из `subscriptions`/`subscription_badge`, **а не из `profiles`** (в `profiles` их нет) — именно они решают подмену слота «батарея ↔ ассет». Инвалидация после мутаций RPC (`submit_task_attempt`, `complete_lesson`, `refresh_user_state`) и после вебхука/`validate-purchase` (статус подписки). При старте/заходе батарею **и** стрик актуализирует `refresh_user_state` (§8.1).
- **Форматирование чисел в HUD.** Числа в слотах `level`, `streak`, `diamonds`, `battery` форматируются по правилу:
  - 0–999 → показываем как есть (`N`).
  - 1 000–9 999 → `1.2k` (одна десятая, если дробная часть ненулевая; иначе `1k`).
  - 10 000+ → `12k` (без дроби).
  - Стрик: то же правило; практический потолок `999+` (≥1000 дней — исключение, но обрабатывается).
  - Реализуется утилитой `formatCompact(n: number): string` (`src/core/utils/format.ts`). Числовой слот — фиксированной ширины (`minWidth`), чтобы HUD не прыгал при смене разрядности.
- **Верстка и safe area.** HUD рендерится с учётом `useSafeAreaInsets().top` (Dynamic Island, notch). **Стрелка «назад» не является частью HUD** — она рендерится в хедере экрана ниже HUD (§10.1) и никогда не конкурирует за горизонталь с элементами HUD.
- **Интерактивность (тап по элементам):**
  ...
  - **Тап по `diamonds`** → **ничего** (неинтерактивно — по замыслу: алмазы пассивный счётчик, баланс очевиден без подсказки).
  - **Тап по `streak`** → **выпадающее сверху окошко** (дропдаун): «🔥 Current streak: N days» + «🏆 Longest: N days» (данные из `Streak.current` / `Streak.longest`, §9). Главный ретеншн-сюрфейс — пользователь видит свой рекорд и не хочет его потерять. Закрывается **тапом вне окошка**.
...
- **Повышение уровня (level-up):** при `complete_lesson` с `leveled_up=true` (каждые `coursesPerLevel` курсов, §A4) поверх всего показывается **полноэкранное окно-оверлей** — как экран награды в конце урока, но с **другим оформлением** маскота (в MVP — статичный портрет, как §12.6; отдельная Rive-анимация level-up — пост-MVP) и сообщением «Level up! You reached level N». **Закрывается тапом по окну** (Gap #8), без кнопки. (Это отдельный оверлей, не `reward`-экран.) **Очерёдность:** если завершение урока дало и награду, и уровень, окна идут **последовательно** — сначала `reward`, затем level-up, затем `course_path`; каждое снимается тапом (два тапа закроют оба; быстрый двойной тап проскочит оба — §12.6).
- **UiState HUD:** часть состояния каркаса; при отсутствии данных — последнее кэшированное значение (MMKV), без блокировки экрана.
- **Анимации HUD — канон [22-motion.md §5](22-motion.md):** count-up/down алмазов с ледяным шиммером, молчаливый тик батареи + «тление» `loop-ember` при `battery-low`, смерть стрика (HUD-потёк + тост), тост автозаморозки, scale-pop уровня. Для тостов `refresh_user_state` должен возвращать события `streak_broken` и `freeze_applied{days, diamonds_spent}` (§8.1) — иначе клиенту не из чего собрать тост.

### 12.1 Splash

- **Назначение:** заставка на цветном фоне; проверка сессии; загрузка конфига и economy; **проверка минимальной версии** (force-update, §A15). Нативный splash (`expo-splash-screen`) удерживается до готовности JS, затем — JS-сплэш. **В MVP — статичный кадр** (портрет Распутина проявляется из тьмы, `expo-image`, §brand.md §2). **Rive-сплэш отложен пост-MVP** (слот Rive-ready: `<Image>` → `<Rive>` без правок логики).
...
- **Онбординг — перед выбором стартовой вкладки (канон [35-onboarding.md](35-onboarding.md)).** После **первого** успешного входа, если локальный MMKV-флаг `onboarding_completed` не стоит → роут `onboarding` (`fullScreenModal`, HUD/таб-бар скрыты) **до** приземления на стартовую вкладку: 4 интро-экрана голосом бренда → «Begin» → **ведомый бесплатный первый урок** (`first_lesson_free`) → JIT-споты по HUD. Назначение диплинка (если есть) прокидывается в финальную «Begin». Старая формулировка «новичок после логина сразу попадает на Grid» **заменена** этим флоу; диплинк-приоритет сохраняется (интро всё равно первое, см. ниже и 35).
...
- **Состояние «курс пройден» (overlay, D).** Если `is_completed=true` (из `get_home_feed`) — поверх обложки/градиента карточки кладётся **более тёмный затемняющий overlay** (токен `overlay-completed`, §5.6) и в углу — **значок «пройдено»** (Phosphor `CheckCircle`, токен `success`). Карточка остаётся кликабельной (тап → `course_path`, можно перепройти). Затемнение «гасит» пройденный курс, не пряча его и не мешая соседним карточкам.
...
> **Намеренно разрежённая карточка (ревью #10/#11).** На карточке Lore — **только три элемента**: логотип, пилюля категории, заголовок. **Убраны** счётчики уроков/заданий, кнопка «Get the lore» и декор (`card_decoration_url`) — портрет 3:4 не перегружается, иерархия чистая (в духе разрежённых карточек Duolingo). Счётчики уроков/заданий остаются только на карточке курса в категории (§12.3в) и внутри `course_path`. Поле `card_decoration_url` в этой карточке больше **не используется** (остаётся в модели/БД, но не рендерится — кандидат на удаление при чистке схемы).

- **Действие:** тап по **всей карточке** → `course_path/{course_id}` (отдельной кнопки нет — карточка кликабельна целиком).
- **Состояния:** loading — skeleton-карточки; empty — «No lores yet»; ошибки изображений — placeholder (`expo-image`), не краш (см. A13).

#### 12.3б Discover / Grid (вкладка 3) — инстаграм-сетка обложек
...
- **Состояние «курс пройден» (D):** если `is_completed=true` — поверх обложки ячейки **более тёмный overlay** (`overlay-completed`) + значок «пройдено» (`success`, Phosphor `CheckCircle`). Ячейка остаётся кликабельной (перепрохождение).
...
- **Раскладка — `SectionList` с «липкими» заголовками-буквами:**
  ...
  - **Строка категории — мини-карточка** (в духе кита, не голый текст): маленькая обложка (`expo-image`) + имя категории + счётчик «N courses» + `CaretRight` (Phosphor); фон `card`/`surface`, скругление. Тап → `category/{categoryId}` (§12.3в) в стеке этого же таба.
...
> **Канон раскладки — концепция «Расклад» (The Spread), [36-course-path.md](36-course-path.md).** Этот раздел — краткое резюме; полные детали (синусоида-змейка, рубашка карты, 3D-флип разблокировки, сургучная печать, underglow-пульс активной карты, авто-скролл) — в 36. Роут `course_path/{courseId}`, данные `get_course_path`, состояния `completed/active/locked` и плашка «Course completed» — **без изменений**; меняется только визуальная раскладка и состояния карточек.

- **Назначение:** экран курса — **расклад карт-уроков**: вертикальная **змейка-одиночка** (одна карта на «шаг», смещение по синусоиде), каждая карта = один `lesson` с обложкой (`cover_image_url`). Без нарисованной дороги/линий — маршрут несёт ритм размещения и граница «вскрытые ↑ / рубашки ↓». Тот же роут `course_path/{courseId}`, тот же HUD сверху. Фон экрана — цвет из `path_background_color`.
...
- **Раскладка:** вертикальный скролл, **одна карта на шаг** по `sort_order`; горизонтальное смещение чередуется по синусоиде (амплитуда ~18–22% ширины), лёгкий разворот ±2–3° (детерминированно по `sort_order`). Карта — **вертикальная ~3:4**, ширина ~55–60% экрана (видно 2–2.5 шага), скругление `radiusLarge`, фон = обложка урока (`expo-image`, blurhash/placeholder; ошибка → placeholder, A13). **Угловой индекс игральной карты:** в верхнем левом углу лица — римская цифра (Cinzel) + маленькая «масть»-сигил (капля крови) на локальном scrim; крупные знаки поверх центра обложки запрещены. **Авто-скролл к активной карте** при открытии (в верхнюю треть). Детали — [36-course-path.md §2](36-course-path.md).
- **Плашка «Course completed» (сверху, D).** Если `course.is_completed=true` — над сеткой уроков показывается компактная **плашка-баннер** «Course completed» со значком `success` (Phosphor `CheckCircle`), вписанная в дизайн-кит (фон `surface`/`success`-акцент). Так пользователь, открыв уже пройденный курс, сразу видит, что курс завершён; карточки уроков при этом доступны для перепрохождения.

**Состояния карточки урока (через токены `node-*`, §5.6; без хардкода цвета). Полные детали и анимации — [36-course-path.md §3–§4](36-course-path.md):**

1. **`completed` (пройден).** Карта **вскрыта**: обложка в полном цвете; в углу — маленькая **сургучная печать** с ✓ (токен `success` + иконка, colorblind-safe). Без рамок и анимаций. Тап → можно перепройти урок (`lesson/{id}`). *(Пройденность всего курса показывает верхняя плашка.)*
1. **`active` (текущий) — вскрытая карта с живым свечением.** Обложка в полном цвете, **lift** (scale ~1.05), бейдж-пилюля **«Continue»** (`primary`) и **blood-underglow из-за самой карты** (свет «пробивается» сзади, лежит **под** картой по z-index). Карта **пульсирует масштабом синхронно со свечением** в ритме «сердцебиение» (`loop-heartbeat`, цикл ~1.2 с). Контурные блики/beam по рамке **запрещены**. Единственная такая карта на экране; в змейке вокруг воздух, поэтому свечение никому не мешает (ограничение «без свечения» из сетки 2×N здесь **снято**). Reduce-motion → статичное свечение, карта зафиксирована в lift. Тап → `lesson/{id}`.
1. **`locked` (будущий) — рубашка карты.** Карта лежит **рубашкой вверх**: общий статичный ассет (тёмный oxblood-узор, оккультный сигил, тонкая рамка `border`); обложка **не видна вовсе** (вместо прежнего desaturate-overlay). Иконка `Lock` не обязательна (рубашка читается сама; опц. маленький замок в углу). **Неинтерактивна:** тап ничего не делает. **Момент разблокировки** (возврат с `reward`): карта **переворачивается** 3D-флипом рубашка→обложка (`path-flip` = `dur-drama` 600 ms, `ease-emphasized`), затем проявляются рамка и бейдж — второй дофаминовый пик после награды.
...
- **Аудио и автостарт (спящая фича, D-1):** озвучка работает, **только** когда включён глобальный тумблер `app_config.economy.reading_tts_enabled` (дефолт `false`, включается из админки B6.8). При `true`: переключаясь на сегмент, озвучка текущего раздела (если `audio_url != null`) **стартует сразу** (`expo-audio`). При `false` — аудио не запускается вообще, даже если `audio_url` заполнен (пользователь читает сам). Источник истины по аудио — inline `payload.segments[].audio_url` (§6.2).
...
> **`Sound Effects`** (Preferences) управляет **только** этими эффект-звуками; **TTS-озвучка чтения под него не попадает** (TTS играет при включённом `reading_tts_enabled` и наличии `audio_url`, §12.5а — спящая фича). **`Vibration`** управляет всей хаптикой. Источник эффект-звуков — бандл-ассеты приложения (`expo-audio`).
...
- **Назначение:** маскот (Распутин) + текст похвалы + начисленная награда + разблокировка следующей ноды. **Полноэкранное окно-оверлей поверх всего; закрывается тапом по окну** (Gap #8), без кнопки `Continue`. Данные — из ответа `complete_lesson` (§8.1). **Хореография — канон [22-motion.md §6.1](22-motion.md):** blood-wipe → маскот → count-up алмазов (ледяной шиммер, haptic) → ember-вспышка стрика; окно неактивности `lock-reward` 450 ms после последнего бита перед tap-to-dismiss.
- **Маскот в MVP — статичный портрет (без Rive).** В MVP показывается **статичный портрет** Распутина (`expo-image`, арт-дирекция §brand.md §2: лицо из тьмы, гипнотичный взгляд, одна кровь-деталь). **Rive-анимация маскота отложена пост-MVP** (idle-дыхание/взгляд, celebrate/encourage-пулы, геро-момент с вспышкой глаз). Слот Rive-ready: позже статичный `<Image>` меняется на `<Rive>` без правок логики награды.
...
- **Текущее поведение (MVP):** статичный портрет маскота (анимация-пул отложен пост-MVP, см. выше). **Урок как таковой не «провален/пройден»** (см. A7).
...
### 12.13 Onboarding «Посвящение» — *канон в [35-onboarding.md](35-onboarding.md)*

- **Назначение:** флоу первого запуска — 4 интро-экрана голосом бренда (маскот-Распутин) + **ведомый бесплатный первый урок** + JIT-споты по HUD (объяснение батарея/алмазы/стрик в момент первого появления). Полная спецификация (экраны, тексты, граничные случаи, сдвиг монетизации на ~3-й урок) — в [35-onboarding.md](35-onboarding.md).
- **Роут/триггер:** `onboarding` (`fullScreenModal`, HUD/таб-бар скрыты), после первого входа при отсутствии MMKV-флага `onboarding_completed`, **до** выбора стартовой вкладки (§12.1). Диплинк-назначение прокидывается в «Begin».
- **UiState:** `intro(step) | startingLesson`; reduce-motion-фолбэк.
- **Хранение:** MMKV — `onboarding_completed`, `seen_tip_diamonds/streak/battery`; серверно (`profiles`) — `first_lesson_free_used bool` (защита от повтора через reinstall). Конфиг — `economy.first_lesson_free` (дефолт true), `app_config.onboarding_course_id` (курируемый стартовый курс).
