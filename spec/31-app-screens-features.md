...
- **Раскладка:** вертикальный бесконечный скролл, **3 колонки**, ячейки — **вертикальные** (портрет ~9:16), плотная сетка (минимальные/нулевые отступы, как Instagram). Ячейка = обложка **первого урока** курса = `lessons.cover_image_url` урока с минимальным `sort_order` («первое видео» курса; генерируется конвейером B3 `lesson_cover`, Приложение C — тот же кадр, что зацепил пользователя в соц-сети). `expo-image` (кэш/blurhash/placeholder; ошибка → placeholder, не краш, A13).
