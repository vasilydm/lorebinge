...
Обучающее приложение по истории в стиле Duolingo. Маркетинговая воронка: «затягивающие» ИИ-видео в соцсетях → крючок → установка приложения → обучение по курсам из уроков и заданий (внутри курса — сетка карточек-уроков, §12.4). Геймификация: батарейка (лимит тестов), алмазы (валюта; зарабатываются за уроки, тратятся только на автозаморозку стрика), стрик (дни подряд), уровень (за пройденные курсы), маскот-награды, подписка. *(Квесты, ачивки и магазин убраны из MVP — глобальные решения / ВАЖН-16 / КРИТ-8.)*
...
└── src/
    ...
    │   ├── reward/              # маскот + награды-оверлеи + окно level-up (закрытие тапом, §12.0/§12.6)
    │   ├── subscription/        # вкладка Subscription + paywall-оверлей (§12.8/§12.9)
    │   ├── profile/             # вкладка Profile (только инфо профиля; квесты убраны) + settings
