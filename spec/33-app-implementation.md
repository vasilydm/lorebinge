...
- **Перепрохождение (ВАЖН-15):** если урок уже `completed`, `complete_lesson` начисляет фикс `lesson_replay_diamond_reward` (дефолт **1**, настраивается в админке; `reason='lesson_replay_reward'`), **без** заряда батареи, и возвращает `already_rewarded=true`. Взвешенный рандом при повторе не разыгрывается. Анти-фарм не цель (NFR/НЕКР-15).
...
- **Компромисс по прогонам (ВАЖН-2).** Сервер **не отслеживает отдельные прогоны** урока: момент вызова `complete_lesson` контролирует клиент (осознанное упрощение, анти-чит не цель). Attempts от брошенного прогона остаются в БД. Чтобы тон маскота считался за один заход, `errors_count` сбрасывается в начале захода через `start_lesson(lesson_id)` (§8.1), который также лениво создаёт `user_course_progress` (ВАЖН-1). Кнопки «Restart» нет — перепрохождение = повторный заход в `completed`-урок с уменьшенной наградой (§A6).
