...
- **Подписчики (ключевой смысл монетизации):**
  ...
  - При **отмене/истечении** подписки (статус → `expired`/`cancelled`/`none`) батарея **возвращается**: HUD снова показывает батарею. На момент возврата `battery` устанавливается в `battery_max` (полная), `last_battery_refill_at = now()`. **Исполнитель — `refresh_user_state`** (лениво, при обнаружении перехода `trial`/`active` → `expired`/`cancelled`/`none`; C-1, §8.1). Это **единственное** место сброса — переживает отсутствие/задержку вебхука; `revenuecat-webhook` лишь обновляет `subscriptions.status`.
...
- **Критерий завершения — курсор, а не наличие attempts (C-2).** `complete_lesson` проверяет, что `user_lesson_progress.current_task_index` дошёл до **последнего** задания урока (по `sort_order`). Курсор двигает `submit_task_attempt` по мере прохождения (тест-задания и контентные — одинаково: контентное «Continue»/конец видео тоже продвигает индекс). Прежняя проверка «у каждой задачи есть хотя бы один `user_task_attempts`» **отменена**: `user_task_attempts` накопительны и не удаляются, поэтому на **перепрохождении** уже `completed`-урока такая проверка проходила мгновенно, до реального ответа (ложный guard). Курсор сбрасывается в 0 при свежем входе/перепрохождении, поэтому честно отражает текущий проход.
- **Перепрохождение — косметика, ни на что не влияет (подтверждено заказчиком).** Повторный заход в `completed`-урок нужен «просто так, если пользователь хочет»: награда фикс-`lesson_replay_diamond_reward` (дефолт 1) и идемпотентна (§6.7/§A6), экономика/разблокировка не меняются. `user_task_attempts` при этом продолжают накапливаться как история ответов (для аналитики), завершение определяет только курсор.
