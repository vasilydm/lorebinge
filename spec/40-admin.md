...
Визуальный конструктор: вкладки навигации и блоки внутри них; вкл/выкл, переупорядочивание (dnd), перенос блока между вкладками; привязка к `min_app_version`; запись в `ui_configs`; гарантия одного активного конфига на диапазон версий. **Вкладка `Catalog`** (алфавитный индекс категорий, блок `CATALOG`, §12.3г приложения) поставляется в сборке, но **по умолчанию выключена** (`enabled:false`) — именно здесь её включают тумблером, когда каталог курсов разрастётся (пока курсов мало, на Home/Grid хватает лент). Включение применяется на следующем холодном старте (A11), без обновления приложения.

### B6.8 Economy

Редактор `app_config.economy`: стартовые алмазы (300), `battery_max` (10), `battery_cost_per_answer`, `battery_refill_minutes` *(поле `battery_reward_per_lesson` убрано — бонуса заряда за завершение урока больше нет, награда за урок только алмазами; Gap #6/§A4/§12.6)*, **`lesson_diamond_rewards`** (редактируемый список «сумма → вес» для рандомной награды за **первое** прохождение, дефолт 10/15/20 с весами 70/25/5, §A6), **`lesson_replay_diamond_reward`** (фикс за перепрохождение, дефолт 1, ВАЖН-15), **`courses_per_level`** (курсов на +1 уровень, дефолт 5, §12.0), **`streak_freeze_cost_per_day`** (дефолт 150 — сколько алмазов сервер **автоматически списывает** за пропущенный день; магазина заморозок нет, КРИТ-8/§A5). **Тумблер `mascot_tone_reaction_enabled`** (дефолт `off`) — включает реакцию маскота на качество заданий (пулы celebrate/encourage по `errors_count`, F7/§12.6); при включении без готовых анимаций клиент мягко откатывается на нейтральный пул. Отдельно — `app_config.min_supported_build` (force-update, §A15) и загрузка **ассета-замены батареи** (`app_config.subscription_badge`: `asset_url` в Storage, `asset_type` rive/lottie/svg/png, опц. `label`). Изменения версионируются/логируются. *(Квестов, ачивок и магазина в админке нет — убраны.)*
